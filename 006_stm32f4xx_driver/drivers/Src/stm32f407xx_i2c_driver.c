...
static void i2c_master_handle_RXNE_interrupt(i2c_handle_t *arg_poi2c_handler) {
	...
	if (0U == arg_poi2c_handler->m_u32rx_len) {
		...
		/*!< Close the i2c rx >*/
		i2c_close_receive_data(arg_poi2c_handler);
...
void i2c_close_send_data(i2c_handle_t *arg_poi2c_handler) {
	...
	arg_poi2c_handler->m_poi2c_reg->CR2 &= ~(SET << I2C_CR2_ITBUFEN);

	/*!< Implement the code to disable ITEVFEN Control bit >*/
	arg_poi2c_handler->m_poi2c_reg->CR2 &= ~(SET << I2C_CR2_ITEVTEN);

	arg_poi2c_handler->m_u8rx_tx_state = I2C_READY;
	arg_poi2c_handler->m_pu8tx_buffer = NULL;
	arg_poi2c_handler->m_u32tx_len = 0U;
}
...
void i2c_close_receive_data(i2c_handle_t *arg_poi2c_handler) {
	...
	arg_poi2c_handler->m_u8rx_tx_state = I2C_READY;
	arg_poi2c_handler->m_pu8rx_buffer = NULL;
	arg_poi2c_handler->m_u32rx_size = 0U;
	arg_poi2c_handler->m_u32rx_len = 0U;

	if (I2C_ACK_EN == arg_poi2c_handler->m_poi2c_sconfig->ack_control) {
		i2c_manage_ack(arg_poi2c_handler->m_poi2c_reg, ENABLE);
	}
}
...
void i2c_ev_irq_handler(i2c_handle_t *arg_poi2c_handler) {
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened
		...
		if ((arg_poi2c_handler->m_poi2c_reg->SR2 >> I2C_SR2_MSL) & SET) {
			...
			/*!< slave mode >*/
			if ((arg_poi2c_handler->m_poi2c_reg->SR2 >> I2C_SR2_TRA) & SET) {
				/*!< slave is in TX mode >*/
				i2c_app_callback(arg_poi2c_handler, I2C_EV_DATA_REQ);
			}
		}
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened
		...
		if ((arg_poi2c_handler->m_poi2c_reg->SR2 >> I2C_SR2_MSL) & SET) {
			...
			/*!< slave mode >*/
			if (RESET == ((arg_poi2c_handler->m_poi2c_reg->SR2 >> I2C_SR2_TRA) & SET)) {
				/*!< slave is in RX mode >*/
				i2c_app_callback(arg_poi2c_handler, I2C_EV_DATA_RCV);
			}
		}
	}
}
...
void i2c_er_irq_handler(i2c_handle_t *arg_poi2c_handler) {
	...
	uint8_t loc_u8is_irq_expected = ((arg_poi2c_handler->m_poi2c_reg->CR2
			>> I2C_CR2_ITERREN) & SET);
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		/*!< This is the bus error >*/

		/*!< Clear the bus error flag :
		 * Cleared by software writing 0 >*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_BERR);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_BERR);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		 ...
		 * Cleared by software writing 0>*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_ARLO);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_ARLO);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		 ...
		 * Cleared by software writing 0>*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_AF);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_AF);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		...
		/*!< notify the application >*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_OVR);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		 ...
		 * Cleared by software writing 0 >*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_PECERR);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_PECERR);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		 ...
		 * Cleared by software writing 0 >*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_TIMEOUT);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_TIMEOUT);
	}
	...
	if (loc_u8is_irq_expected && loc_u8is_irq_happened) {
		...
		/*!< Clear the SMBus alert error flag :
		 * Cleared by software writing 0 >*/
		arg_poi2c_handler->m_poi2c_reg->SR1 &= ~(SET << I2C_SR1_SMBALERT);

		/*!< notify the application >*/
		i2c_app_callback(arg_poi2c_handler, I2C_ERROR_SMBALERT);
	}
...
void i2c_irq_slave_manage_callback_events (i2c_reg_t *arg_ptr_i2c, uint8_t arg_enable_or_disable) {
	if (ENABLE == arg_enable_or_disable) {
		arg_ptr_i2c->CR2 |= (SET << I2C_CR2_ITBUFEN);
		arg_ptr_i2c->CR2 |= (SET << I2C_CR2_ITEVTEN);
		arg_ptr_i2c->CR2 |= (SET << I2C_CR2_ITERREN);
	} else {
		arg_ptr_i2c->CR2 &= ~(SET << I2C_CR2_ITBUFEN);
		arg_ptr_i2c->CR2 &= ~(SET << I2C_CR2_ITEVTEN);
		arg_ptr_i2c->CR2 &= ~(SET << I2C_CR2_ITERREN);
...
uint8_t i2c_master_send_data_it(i2c_handle_t *arg_poi2c_handler,
	...
	if (I2C_READY == loc_u8busy_state) {
		...
		arg_poi2c_handler->m_poi2c_reg->CR2 |= (SET << I2C_CR2_ITERREN);
...
void i2c_slave_send_data(i2c_reg_t *arg_poi2c_reg, uint8_t arg_u8data) {
	arg_poi2c_reg->DR = arg_u8data;
}

uint8_t i2c_slave_receive_data(i2c_reg_t *arg_poi2c_reg) {
	return (uint8_t)arg_poi2c_reg->DR;
}
...
__attribute__((weak)) void i2c_app_callback(i2c_handle_t *arg_poi2c_handler,
		uint8_t arg_u8evt) {

...
void i2c_init(i2c_handle_t *arg_ptr_oi2c_handler) {
	...
	} else if (arg_ptr_oi2c_handler->ptr_sconfig->scl_speed
			<= I2C_SCL_SPEED_FM2K) {
		/*!< fast mode*/
...
void i2c_master_send_data(i2c_handle_t *arg_ptr_oi2c_handler,
	...
	while (FLAG_RESET == i2c_get_flag_status(arg_ptr_oi2c_handler->ptr_oi2c,
	I2C_FLAG_ADDR))
		;
	...
	while (FLAG_RESET
			== i2c_get_flag_status(arg_ptr_oi2c_handler->ptr_oi2c, I2C_FLAG_TXE))
		;
	while (FLAG_RESET
			== i2c_get_flag_status(arg_ptr_oi2c_handler->ptr_oi2c, I2C_FLAG_BTF))
		;
...
void i2c_master_receive_data(i2c_handle_t *arg_ptr_oi2c_handler,
	...
	arg_u8slave_addr = (uint8_t) ((arg_u8slave_addr << SHIFT_ONE) | SET);
	arg_ptr_oi2c_handler->ptr_oi2c->DR = arg_u8slave_addr;
	//arg_ptr_oi2c_handler->ptr_oi2c->DR = (uint8_t) (arg_u8slave_addr & SEVEN_BIT);
	...
	if (arg_u32len <= 1U) {
		/*!< 5. Disable acking*/
		i2c_manage_ack(arg_ptr_oi2c_handler->ptr_oi2c, I2C_ACK_DI);
		...
		*arg_ptr_u8rx_buffer = (uint8_t) arg_ptr_oi2c_handler->ptr_oi2c->DR;
	...
	i2c_manage_ack(arg_ptr_oi2c_handler->ptr_oi2c, I2C_ACK_EN);
	}
}

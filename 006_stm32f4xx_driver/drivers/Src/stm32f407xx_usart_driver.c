...
void usart_init(usart_handle_t *arg_pousart_handler) {
	...
	usart_set_baud_rate(arg_pousart_handler->m_poi2c_reg, arg_pousart_handler->m_pousart_config_t->m_u32baud);
...
void usart_send_data(usart_handle_t *arg_pusart_handle,
	...
	while (FLAG_RESET == usart_get_flag_status(arg_pusart_handle->m_poi2c_reg, USART_FLAG_TC));
}
...
void usart_set_baud_rate(usart_reg_t *arg_pusart,
	...
	if(USART1 == arg_pusart || USART6 == arg_pusart) {
		...
		loc_u32pclk = rcc_get_pclk_value(APB2);
	} else {
		loc_u32pclk = rcc_get_pclk_value(APB1);
	}
	...
	loc_u32temp_reg = (loc_u32mantissa << USART_BRR_DIV_MANTISSA);
	...
	/*!< Place the Fraction part in the appropriate bit position Refer USART_BRR >*/
	loc_u32temp_reg |= ((loc_u32fraction & FOUR_BIT) << USART_BRR_DIV_FRACTION);

	arg_pusart->BRR = loc_u32temp_reg;
}
